...
# Parameters
a = 0.2 # Probability that if a prey gets paired with a predator, it will die
...
d = 0.01 # Probability of death for a predator

# Number of turns
num_turns = 5000

# Define grid as 8 by 8 grid of empty lists
col_num = row_num = 30
grid = [[[] for _ in range(col_num)] for _ in range(row_num)]

# Add initial animals
initial_num_predators = 50
initial_num_prey = 50
...
def eating(grid):
    for r, row in enumerate(grid):
        for c, point in enumerate(row):
            ...
            num_pairs = min(len(prey_list), len(predator_list))
